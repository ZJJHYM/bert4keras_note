...
if os.path.exists('seq2seq_config.json'):
    ...
    for a in tqdm(db.find()):
        for w in a['content']:  # 纯文本，不用分词
            chars[w] = chars.get(w, 0) + 1  # 没有拿到这个key的内容的话，就返回0
...
def to_one_hot(x):
    ...
    x, x_mask = x
    x = K.cast(x, 'int32')
    x = K.one_hot(x, len(chars) + 4)  # (btz, seq_len, len(chars) + 4)
    x = K.sum(x_mask * x, 1, keepdims=True) # (btz, seq_len, len(chars) + 4)*(btz, seq_len, 1)  把seq_len这一维坍缩去掉，且相加表示当前这个字如果出现在文章中，则该one-hot中的值是为1
    x = K.cast(K.greater(x, 0.5), 'float32')  # 转成0、1矩阵，(btz, 1, len(chars) + 4)
    return x
...
class SelfModulatedLayerNormalization(OurLayer):
    ...
    def call(self, inputs):
        inputs, cond = inputs # cond：encoder
        inputs = self.reuse(self.layernorm, inputs) # 对当前状态y做一层LN
        beta = self.reuse(self.beta_dense_1, cond) # 对encoder做全连接
        beta = self.reuse(self.beta_dense_2, beta) # 再做一次全连接，得到beta
        gamma = self.reuse(self.gamma_dense_1, cond) # 重复beta的计算过程
        gamma = self.reuse(self.gamma_dense_2, gamma)
        for _ in range(K.ndim(inputs) - K.ndim(cond)):
            beta = K.expand_dims(beta, 1) # (btz, 1 ,128)
            gamma = K.expand_dims(gamma, 1) # (btz, 1 ,128)
...
'''
1、输入准备
制作mask过程，非常有用，可以套用在其他地方 
ScaleShift:学习输出的先验分布

2、encoder部分，输出的是个什么东西？只是一个向量而已

自己写双向LSTM


3、decoder部分，同样输出的也只是一个向量，根据该向量，经过dense层，即可得出概率
自己写一个SelfModulatedLayerNormalization
...
# 搭建seq2seq模型
x_in = Input(shape=(None,))  # (btz, seq_len_x)
y_in = Input(shape=(None,))  # (btz, seq_len_y)
x, y = x_in, y_in

x_mask = Lambda(lambda x: K.cast(K.greater(K.expand_dims(x, 2), 0), 'float32'))(
    x)  # shape=(btz,seq_len,1) 制作mask过程，非常有用，可以套用在其他地方
y_mask = Lambda(lambda x: K.cast(K.greater(K.expand_dims(x, 2), 0), 'float32'))(y)  # shape=(btz,seq_len,1)

x_one_hot = Lambda(to_one_hot)([x, x_mask])  # (btz, 1, len(chars) + 4)
...
# decoder，双层单向LSTM
y = SelfModulatedLayerNormalization(z_dim // 4)([y, x_max]) # (btz, seq_len, 128)
y = CuDNNLSTM(z_dim, return_sequences=True)(y) # (btz, seq_len, 128)
y = SelfModulatedLayerNormalization(z_dim // 4)([y, x_max]) # (btz, seq_len, 128)
y = CuDNNLSTM(z_dim, return_sequences=True)(y) # (btz, seq_len, 128)
y = SelfModulatedLayerNormalization(z_dim // 4)([y, x_max]) # (btz, seq_len, 128)
